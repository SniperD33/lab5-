...
void init_opengl(void)
{
	...
	g.tex.yc[1] = 1.0;
	//
	//this is the small image for testing 
	//
	g.tex2.backImage = &img[0];
	//create opengl texture elements
	glGenTextures(1, &g.tex2.backTexture);
	w = g.tex2.backImage->width;
	h = g.tex2.backImage->height;
	...
	g.tex2.yc[1] = 1.0;
	//
	//shows whole image all at once instead of transitioning like og tex
}
